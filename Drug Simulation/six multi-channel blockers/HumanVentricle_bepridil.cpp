...
class ORdHumanVentricle//:public Cell
{
	...
	double const bep=0.1;

	double BNa= 1-1/(1.0+pow(0.517/bep,1.14));
	double BCaL= 1-1/(1.0+pow(0.157/bep,1.08));
	double BKr= 1-1/(1.0+pow(0.0738/bep,1.33));
	double BNaL= 1-1/(1.0+pow(0.411/bep,1.72));
	double BK1= 1-1/(1.0+pow(66.536/bep,1.0));
	double BKs= 1-1/(1.0+pow(6.156/bep,2.33));
...
//		cansr = 1.2*(1-0.423);
	...
	{		
		...
		#ifdef CORM_Bep
		INa = GNa*sm*sm*sm*sh*sj*(v-ENa)*0.47*BNa;
		#endif

		#ifdef HF_CORM_Bep
		INa = GNa*sm*sm*sm*sh*sj*(v-ENa)*(1-0.741)*0.47*BNa;
		#endif
		...
		#ifdef CORM_Bep
		INaL=GNaL*(v-ENa)*mL*((1.0-fINaLp)*hL+fINaLp*hLp)*2.05*BNaL;
		#endif

		#ifdef HF_CORM_Bep
		INaL=GNaL*(v-ENa)*mL*((1.0-fINaLp)*hL+fINaLp*hLp)*(1+1.034)*2.05*BNaL;
		#endif	
		...
		#if defined (CON) || defined(Baseline) || defined(CORM) || defined(CORM_Bep)
		Ito=Gto*(v-EK)*((1.0-fItop)*a*i+fItop*ap*ip);
		#endif	

		#if defined (HF) || defined(HF_CORM)|| defined(HF_CORM_Bep)
		if(celltype==ENDO)
		...
		#if defined (CORM_Bep) || defined (HF_CORM_Bep)
		ICaL=((1.0-fICaLp)*PCa*PhiCaL*d*(f*(1.0-nca)+jca*fca*nca)+fICaLp*PCap*PhiCaL*d*(fp*(1.0-nca)+jca*fcap*nca))*0.47*BCaL;
		#endif
		...
		#ifdef CORM_Bep
		IKr=GKr*sqrt(ko/5.4)*xr*rkr*(v-EK)*0.56*BKr;
		#endif

		#ifdef HF_CORM_Bep
		if(celltype==0)
			IKr=GKr*sqrt(ko/5.4)*xr*rkr*(v-EK)*(1-0.273)*0.56*BKr;
		if(celltype==1)
			IKr=GKr*sqrt(ko/5.4)*xr*rkr*(v-EK)*(1-0.459)*0.56*BKr;
		if(celltype==2)
			IKr=GKr*sqrt(ko/5.4)*xr*rkr*(v-EK)*0.56*BKr;
		#endif
		...
		#ifdef CORM_Bep
		IKs=GKs*KsCa*xs1*xs2*(v-EKs)*BKs;
		#endif

		#ifdef HF_CORM_Bep
		if(celltype==0)
			IKs=GKs*KsCa*xs1*xs2*(v-EKs)*(1-0.577)*BKs;
		if(celltype==1)
			IKs=GKs*KsCa*xs1*xs2*(v-EKs)*(1-0.594)*BKs;
		if(celltype==2)
			IKs=GKs*KsCa*xs1*xs2*(v-EKs)*(1-0.495)*BKs;
		#endif
		...
		#ifdef CORM_Bep
		IK1=GK1*sqrt(ko)*rk1*xk1*(v-EK)*0.66*BK1;
		#endif

		#ifdef HF_CORM_Bep
		if(celltype==0)
			IK1=GK1*sqrt(ko)*rk1*xk1*(v-EK)*(1-0.550)*0.66*BK1;
		if(celltype==1)
			IK1=GK1*sqrt(ko)*rk1*xk1*(v-EK)*(1-0.553)*0.66*BK1;
		if(celltype==2)
			IK1=GK1*sqrt(ko)*rk1*xk1*(v-EK)*(1-0.527)*0.66*BK1;
		#endif
