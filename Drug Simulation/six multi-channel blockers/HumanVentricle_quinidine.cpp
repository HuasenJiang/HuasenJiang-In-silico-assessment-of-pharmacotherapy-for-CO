...
class ORdHumanVentricle//:public Cell
{
	...
	double const qui=1;
	double qKs= log10(qui*1e-6);

	double QNa= 1-1/(1.0+pow(17/qui,0.92));
	double QCaL= 1-1/(1.0+pow(14.9/qui,1.1));
	double QKr= 1-1/(1.0+pow(0.41/qui,0.76));
	double QNaL= 1-1/(1.0+pow(12.0/qui,1.0));
	double Qto= 1-1/(1.0+pow(21.8/qui,0.67));
	double QK1= 1-1/(1.0+pow(42.6/qui,0.25));
	double QKs= 1/(1.0+pow(-4.34/qKs,17.50));
...
//		cansr = 1.2;
	...
	{		
		...
		#ifdef CORM_Qui
		INa = GNa*sm*sm*sm*sh*sj*(v-ENa)*0.47*QNa;
		#endif

		#ifdef HF_CORM_Qui
		INa = GNa*sm*sm*sm*sh*sj*(v-ENa)*(1-0.741)*0.47*QNa;
		#endif
		...
		#ifdef CORM_Qui
		INaL=GNaL*(v-ENa)*mL*((1.0-fINaLp)*hL+fINaLp*hLp)*2.05*QNaL;
		#endif

		#ifdef HF_CORM_Qui
		INaL=GNaL*(v-ENa)*mL*((1.0-fINaLp)*hL+fINaLp*hLp)*(1+1.034)*2.05*QNaL;
		#endif	
		...
		#ifdef CORM_Qui
		Ito=Gto*(v-EK)*((1.0-fItop)*a*i+fItop*ap*ip)*Qto;
		#endif

		#ifdef HF_CORM_Qui
		if(celltype==0)
			Ito=Gto*(v-EK)*((1.0-fItop)*a*i+fItop*ap*ip)*(1-0.508)*Qto;
		if(celltype==1)
			Ito=Gto*(v-EK)*((1.0-fItop)*a*i+fItop*ap*ip)*(1-0.404)*Qto;
		if(celltype==2)
			Ito=Gto*(v-EK)*((1.0-fItop)*a*i+fItop*ap*ip)*(1-0.376)*Qto;
		#endif		
		...
		#if defined (CORM_Qui) || defined (HF_CORM_Qui)
		ICaL=((1.0-fICaLp)*PCa*PhiCaL*d*(f*(1.0-nca)+jca*fca*nca)+fICaLp*PCap*PhiCaL*d*(fp*(1.0-nca)+jca*fcap*nca))*0.47*QCaL;
		#endif
		...
		#ifdef CORM_Qui
		IKr=GKr*sqrt(ko/5.4)*xr*rkr*(v-EK)*0.56*QKr;
		#endif

		#ifdef HF_CORM_Qui
		if(celltype==0)
			IKr=GKr*sqrt(ko/5.4)*xr*rkr*(v-EK)*(1-0.273)*0.56*QKr;
		if(celltype==1)
			IKr=GKr*sqrt(ko/5.4)*xr*rkr*(v-EK)*(1-0.459)*0.56*QKr;
		if(celltype==2)
			IKr=GKr*sqrt(ko/5.4)*xr*rkr*(v-EK)*0.56*QKr;
		#endif
		...
		#ifdef CORM_Qui
		IKs=GKs*KsCa*xs1*xs2*(v-EKs)*QKs;
		#endif

		#ifdef HF_CORM_Qui
		if(celltype==0)
			IKs=GKs*KsCa*xs1*xs2*(v-EKs)*(1-0.577)*QKs;
		if(celltype==1)
			IKs=GKs*KsCa*xs1*xs2*(v-EKs)*(1-0.594)*QKs;
		if(celltype==2)
			IKs=GKs*KsCa*xs1*xs2*(v-EKs)*(1-0.495)*QKs;
		#endif
		...
		#ifdef CORM_Qui
		IK1=GK1*sqrt(ko)*rk1*xk1*(v-EK)*0.66*QK1;
		#endif

		#ifdef HF_CORM_Qui
		if(celltype==0)
			IK1=GK1*sqrt(ko)*rk1*xk1*(v-EK)*(1-0.550)*0.66*QK1;
		if(celltype==1)
			IK1=GK1*sqrt(ko)*rk1*xk1*(v-EK)*(1-0.553)*0.66*QK1;
		if(celltype==2)
			IK1=GK1*sqrt(ko)*rk1*xk1*(v-EK)*(1-0.527)*0.66*QK1;
		#endif
		...
		#if defined (CON) || defined(Baseline) || defined(CORM)|| defined(CORM_Qui)
		INaCa=INaCa_i+INaCa_ss;
		#endif	

		#if defined (HF) || defined (HF_CORM)|| defined(HF_CORM_Qui)
