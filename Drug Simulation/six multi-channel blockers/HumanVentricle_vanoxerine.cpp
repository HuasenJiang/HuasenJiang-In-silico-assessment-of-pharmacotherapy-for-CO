...
class ORdHumanVentricle//:public Cell
{
	...
	double const vano=0.005;

	double VNa= 1-1/(1.0+pow(0.0346/vano,0.97));
	double VCaL= 1-1/(1.0+pow(0.0162/vano,0.63));
	double VKr= 1-1/(1.0+pow(0.0093/vano,1.11));
	double VNaL= 1-1/(1.0+pow(0.0852/vano,1.62));
	double Vto= 1-1/(1.0+pow(2.0/vano,1.0));
	double VK1= 1-1/(1.0+pow(98.142/vano,1.0));
	double VKs= 1-1/(1.0+pow(2.9/vano,1.0));
...
//		cansr = 1.2;
	...
	{		
		...
		#ifdef CORM_Vano
		INa = GNa*sm*sm*sm*sh*sj*(v-ENa)*0.47*VNa;
		#endif

		#ifdef HF_CORM_Vano
		INa = GNa*sm*sm*sm*sh*sj*(v-ENa)*(1-0.741)*0.47*VNa;
		#endif
		...
		#ifdef CORM_Vano
		INaL=GNaL*(v-ENa)*mL*((1.0-fINaLp)*hL+fINaLp*hLp)*2.05*VNaL;
		#endif

		#ifdef HF_CORM_Vano
		INaL=GNaL*(v-ENa)*mL*((1.0-fINaLp)*hL+fINaLp*hLp)*(1+1.034)*2.05*VNaL;
		#endif	
		...
		#ifdef CORM_Vano
		Ito=Gto*(v-EK)*((1.0-fItop)*a*i+fItop*ap*ip)*Vto;
		#endif

		#ifdef HF_CORM_Vano
		if(celltype==0)
			Ito=Gto*(v-EK)*((1.0-fItop)*a*i+fItop*ap*ip)*(1-0.508)*Vto;
		if(celltype==1)
			Ito=Gto*(v-EK)*((1.0-fItop)*a*i+fItop*ap*ip)*(1-0.404)*Vto;
		if(celltype==2)
			Ito=Gto*(v-EK)*((1.0-fItop)*a*i+fItop*ap*ip)*(1-0.376)*Vto;
		#endif		
		...
		#if defined (CORM_Vano) || defined (HF_CORM_Vano)
		ICaL=((1.0-fICaLp)*PCa*PhiCaL*d*(f*(1.0-nca)+jca*fca*nca)+fICaLp*PCap*PhiCaL*d*(fp*(1.0-nca)+jca*fcap*nca))*0.47*VCaL;
		#endif
		...
		#ifdef CORM_Vano
		IKr=GKr*sqrt(ko/5.4)*xr*rkr*(v-EK)*0.56*VKr;
		#endif

		#ifdef HF_CORM_Vano
		if(celltype==0)
			IKr=GKr*sqrt(ko/5.4)*xr*rkr*(v-EK)*(1-0.273)*0.56*VKr;
		if(celltype==1)
			IKr=GKr*sqrt(ko/5.4)*xr*rkr*(v-EK)*(1-0.459)*0.56*VKr;
		if(celltype==2)
			IKr=GKr*sqrt(ko/5.4)*xr*rkr*(v-EK)*0.56*VKr;
		#endif
		...
		#ifdef CORM_Vano
		IKs=GKs*KsCa*xs1*xs2*(v-EKs)*VKs;
		#endif

		#ifdef HF_CORM_Vano
		if(celltype==0)
			IKs=GKs*KsCa*xs1*xs2*(v-EKs)*(1-0.577)*VKs;
		if(celltype==1)
			IKs=GKs*KsCa*xs1*xs2*(v-EKs)*(1-0.594)*VKs;
		if(celltype==2)
			IKs=GKs*KsCa*xs1*xs2*(v-EKs)*(1-0.495)*VKs;
		#endif
		...
		#ifdef CORM_Vano
		IK1=GK1*sqrt(ko)*rk1*xk1*(v-EK)*0.66*VK1;
		#endif

		#ifdef HF_CORM_Vano
		if(celltype==0)
			IK1=GK1*sqrt(ko)*rk1*xk1*(v-EK)*(1-0.550)*0.66*VK1;
		if(celltype==1)
			IK1=GK1*sqrt(ko)*rk1*xk1*(v-EK)*(1-0.553)*0.66*VK1;
		if(celltype==2)
			IK1=GK1*sqrt(ko)*rk1*xk1*(v-EK)*(1-0.527)*0.66*VK1;
		#endif
		...
		#if defined (CON) || defined(Baseline) || defined(CORM)|| defined(CORM_Vano)
		INaCa=INaCa_i+INaCa_ss;
		#endif	

		#if defined (HF) || defined (HF_CORM)|| defined(HF_CORM_Vano)
