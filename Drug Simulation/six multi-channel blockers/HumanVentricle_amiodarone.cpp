...
class ORdHumanVentricle//:public Cell
{
	...
	double const ami=0.005;

	double ANa= 1-1/(1.0+pow(40.4/ami,0.75));
	double ACaL= 1-1/(1.0+pow(5.8/ami,1.0));
	double AKr= 1-1/(1.0+pow(0.03/ami,1.0));
	double AKs= 1-1/(1.0+pow(3.84/ami,0.63));
	double ANaK= 1-1/(1.0+pow(15.6/ami,1.0));
	double ANaCa= 1-1/(1.0+pow(3.3/ami,1.0));
...
//		cansr = 1.2*(1-0.423);//HF
	...
	{		
		...
		#ifdef CORM_Ami
		INa = GNa*sm*sm*sm*sh*sj*(v-ENa)*0.47*ANa;
		#endif

		#ifdef HF_CORM_Ami
		INa = GNa*sm*sm*sm*sh*sj*(v-ENa)*(1-0.741)*0.47*ANa;
		#endif
		...
		#if defined (CON) || defined(Baseline) || defined(CORM) || defined(CORM_Ami)
		Ito=Gto*(v-EK)*((1.0-fItop)*a*i+fItop*ap*ip);
		#endif	

		#if defined (HF) || defined(HF_CORM)|| defined(HF_CORM_Ami)
		if(celltype==ENDO)
		...
		#if defined (CORM_Ami) || defined (HF_CORM_Ami)
		ICaL=((1.0-fICaLp)*PCa*PhiCaL*d*(f*(1.0-nca)+jca*fca*nca)+fICaLp*PCap*PhiCaL*d*(fp*(1.0-nca)+jca*fcap*nca))*0.47*ACaL;
		#endif
		...
		#ifdef CORM_Ami
		IKr=GKr*sqrt(ko/5.4)*xr*rkr*(v-EK)*0.56*AKr;
		#endif

		#ifdef HF_CORM_Ami
		if(celltype==0)
			IKr=GKr*sqrt(ko/5.4)*xr*rkr*(v-EK)*(1-0.273)*0.56*AKr;
		if(celltype==1)
			IKr=GKr*sqrt(ko/5.4)*xr*rkr*(v-EK)*(1-0.459)*0.56*AKr;
		if(celltype==2)
			IKr=GKr*sqrt(ko/5.4)*xr*rkr*(v-EK)*0.56*AKr;
		#endif
		...
		#ifdef CORM_Ami
		IKs=GKs*KsCa*xs1*xs2*(v-EKs)*AKs;
		#endif

		#ifdef HF_CORM_Ami
		if(celltype==0)
			IKs=GKs*KsCa*xs1*xs2*(v-EKs)*(1-0.577)*AKs;
		if(celltype==1)
			IKs=GKs*KsCa*xs1*xs2*(v-EKs)*(1-0.594)*AKs;
		if(celltype==2)
			IKs=GKs*KsCa*xs1*xs2*(v-EKs)*(1-0.495)*AKs;
		#endif
		...
		if (celltype==ENDO || celltype==EPI)
		{
			INaCa=(INaCa_i+INaCa_ss)*(1+1.314)*ANaCa;
		}
		if (celltype==M)
		{
			INaCa=(INaCa_i+INaCa_ss)*(1+0.9)*ANaCa;
		}
		#endif
		...
		#ifdef CORM_Ami
		INaK=Pnak*(zna*JnakNa+zk*JnakK)*ANaK;
		#endif

		#ifdef HF_CORM_Ami
		INaK=Pnak*(zna*JnakNa+zk*JnakK)*(1-0.402)*ANaK;
		#endif
