...
class ORdHumanVentricle//:public Cell
{
	...
	double const vera=0.3;

	double VNa= 1-1/(1.0+pow(7.211/vera,0.95));
	double VCaL= 1-1/(1.0+pow(0.0794/vera,0.69));
	double VKr= 1-1/(1.0+pow(0.831/vera,1.17));
	double VNaL= 1-1/(1.0+pow(6.094/vera,1.24));
	double VK1= 1-1/(1.0+pow(9.033/vera,1.0));
	double VKs= 1-1/(1.0+pow(65.587/vera,0.92));
...
//		cansr = 1.2;
	...
	{		
		...
		#if defined (CON) || defined(Baseline) || defined(CORM) || defined(CORM_Vera)
		Ito=Gto*(v-EK)*((1.0-fItop)*a*i+fItop*ap*ip);
		#endif	

		#if defined (HF) || defined(HF_CORM)|| defined(HF_CORM_Vera)
		if(celltype==ENDO)
		...
		#if defined (CORM_Vera) || defined (HF_CORM_Vera)
